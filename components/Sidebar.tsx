...
const Sidebar: React.FC = () => {
  const adminItems = [
    ...
    { name: 'Campaigns', icon: <LayersIcon className="w-4 h-4" /> },
  ...
  return (
    <aside className="w-72 bg-white border-r border-gray-100 hidden lg:flex flex-col h-screen p-8 overflow-y-auto scrollbar-hide">
      {/* User Info Section - Restored from previous design */}
      ...
      <div className="space-y-8 flex-1">
        ...
        <section>
          <nav className="space-y-1">
            <a href="#" className="flex items-center gap-3 px-1 py-2 font-bold text-[13px] text-brand-dark hover:text-brand-accent transition-all">
        ...
        {/* Admin Group - Restored header style */}
        <section>
          ...
          <nav className="space-y-0.5">
            {adminItems.map((item) => (
              <a 
                key={item.name}
                href="#" 
                className="flex items-center gap-3 px-1 py-2 transition-all font-semibold text-[13px] text-brand-gray hover:text-brand-accent"
              >
