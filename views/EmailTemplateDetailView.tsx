...
import { EmailTemplate } from './EmailTemplatesView';

interface EmailTemplateDetailViewProps {
  template: EmailTemplate | null; // null = creating new
  onBack: () => void;
}

const EmailTemplateDetailView: React.FC<EmailTemplateDetailViewProps> = ({ template, onBack }) => {
  const isNew = !template;
  const [subject, setSubject] = useState(template?.subject || '');
  const [body, setBody] = useState(template?.body || '');
  const [status, setStatus] = useState<'active' | 'archived'>(template?.status || 'active');
  const [attachments, setAttachments] = useState<string[]>(template?.attachments || []);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <button onClick={onBack} className="text-xs font-bold text-brand-gray hover:text-brand-accent flex items-center gap-1 uppercase tracking-widest">
        <span>←</span> Back to Email Templates
      </button>
        ...
        <div className="flex items-center gap-3">
          {!isNew && (
            <button
              ...
              className={`text-[11px] font-bold px-5 py-2.5 rounded-xl transition-all shadow-sm border flex items-center gap-1.5 ${
                status === 'archived'
                  ? 'bg-amber-500 text-white border-amber-500 hover:bg-amber-600'
                  : 'bg-white text-brand-gray border-gray-200 hover:border-amber-400 hover:text-amber-500'
              }`}
            >
              {status === 'archived' && (
      ...
      <div className="bg-white rounded-2xl shadow-panel border border-gray-100 overflow-hidden">
        ...
        <div className="px-8 pb-4">
          <textarea
            ...
            className="w-full min-h-[400px] border border-gray-200 rounded-xl px-5 py-4 text-[14px] font-normal text-brand-dark leading-relaxed focus:ring-1 focus:ring-brand-accent focus:border-brand-accent outline-none transition-all resize-y"
